...
- Users can permanently delete local financial data, debt/loan ledgers, local reminders, and the installation key.
...
Debt/loan planning does not introduce a new collection or sharing path: the data stays on-device and leaves only when the user explicitly creates and shares a password-encrypted backup.
